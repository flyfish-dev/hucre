...
function child(parent: XmlElement | undefined, name: string): XmlElement | undefined {
  return parent?.children.find((node): node is XmlElement =>
    typeof node !== "string" && (node.local || node.tag) === name)
}
function pair(node: XmlElement | undefined, a: string, b: string): [number, number] | undefined {
  ...
  if (!/^[+-]?\d+$/.test(node.attrs[a].trim()) || !/^[+-]?\d+$/.test(node.attrs[b].trim())) return undefined
  const x = Number(node.attrs[a]), y = Number(node.attrs[b])
...
export function readDrawingLayout(anchor: XmlElement): Layout {
  ...
  const kind = local === "twoCellAnchor" ? "twoCell" : local === "oneCellAnchor" ? "oneCell"
    : local === "absoluteAnchor" ? "absolute" : undefined
  if (!kind) return {}
  ...
  const pos = pair(kind === "absolute" ? child(anchor, "pos")
    : editAs === "absolute" ? child(xfrm, "off") : undefined, "x", "y")
